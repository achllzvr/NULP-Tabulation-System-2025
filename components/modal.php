...
<div id="<?= htmlspecialchars($modalId, ENT_QUOTES, 'UTF-8') ?>" class="hidden fixed inset-0 z-40 items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true">
  <div class="bg-white w-full max-w-lg rounded shadow-lg overflow-hidden">
    ...
    <div class="p-4 text-sm" id="<?= htmlspecialchars($modalId, ENT_QUOTES, 'UTF-8') ?>-body">
      <?= $bodyHtml ?? '' ?>
    </div>
    <?php if (!empty($footerHtml)): ?>
      <div class="px-4 py-3 bg-slate-50 border-t border-slate-200 text-right" id="<?= htmlspecialchars($modalId, ENT_QUOTES, 'UTF-8') ?>-footer">
        <?= $footerHtml ?>
      </div>
    <?php endif; ?>
  </div>
