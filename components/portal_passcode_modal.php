...
<?php
  // Optional: define a server-side passcode constant; fallback retained for now.
  if (!defined('PORTAL_ACCESS_PASSCODE')) {
    define('PORTAL_ACCESS_PASSCODE', 'NULPANASYS2025');
  }
?>
<div id="portalPasscodeModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 hidden" data-passcode="<?php echo htmlspecialchars(PORTAL_ACCESS_PASSCODE, ENT_QUOTES); ?>">
...
(function() {
  ...
  proceedBtn.onclick = function() {
    const requiredCode = modal.getAttribute('data-passcode');
    if (passcodeInput.value === requiredCode) {
