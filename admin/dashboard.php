...
// Get judges count
$stmt = $conn->prepare("SELECT COUNT(u.id) as count FROM users u JOIN pageant_users pu ON u.id = pu.user_id WHERE pu.pageant_id = ? AND pu.role = 'judge' AND u.is_active = 1");
$stmt->bind_param("i", $pageant_id);
$stmt->execute();
