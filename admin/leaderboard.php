...
?>
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-slate-200">Real-time participant rankings and scores</p>
        </div>
        <div class="flex gap-3">
          <button onclick="refreshLeaderboard()" class="bg-blue-500 bg-opacity-30 hover:bg-blue-600 hover:bg-opacity-40 text-white font-medium px-6 py-3 rounded-lg transition-colors flex items-center gap-2 border border-white border-opacity-20 backdrop-blur-md">
  ...
  <p class="text-lg font-bold text-white mb-1"><?php echo ($current_leader && isset($current_leader['full_name'])) ? htmlspecialchars($current_leader['full_name']) : 'TBD'; ?></p>
  <p class="text-sm text-slate-200"><?php echo ($current_leader && isset($current_leader['total_score'])) ? 'Score: ' . number_format($current_leader['total_score'], 2) : 'When scores available'; ?></p>
      </div>
  ...
  <div class="bg-white bg-opacity-15 backdrop-blur-md rounded-xl shadow-sm border border-white border-opacity-20 mb-8">
      <div class="px-6 py-4 border-b border-white border-opacity-10">
        <h3 class="text-lg font-semibold text-white">Filters & Options</h3>
      ...
      <div class="p-6">
        <div class="grid md:grid-cols-3 gap-6">
          <div>
            <label class="block text-sm font-medium text-slate-200 mb-2">Round Filter</label>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-200 mb-2">View Mode</label>
            <select id="viewMode" class="w-full bg-white bg-opacity-20 backdrop-blur-sm border border-white border-opacity-30 rounded-lg px-4 py-3 text-sm text-white focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-colors" onchange="changeViewMode()">
        ...
        <div id="leaderboardContent">
          <?php if (!empty($rows)): ?>
            ...
            <table class="w-full">
              <thead class="bg-white bg-opacity-10 border-b border-white border-opacity-10">
                <tr>
                  <th class="px-6 py-4 text-left text-sm font-semibold text-white rank-column">Rank</th>
                  ...
                  <th class="px-6 py-4 text-center text-sm font-semibold text-white score-details" style="display: none;">Details</th>
              ...
              <tbody class="divide-y divide-white divide-opacity-5">
                <?php foreach ($rows as $index => $participant): ?>
                  <tr class="<?php echo $index % 2 === 0 ? 'bg-white bg-opacity-10' : 'bg-white bg-opacity-5'; ?> hover:bg-blue-500 hover:bg-opacity-10 transition-colors duration-200">
                      ...
                      <div class="flex items-center">
                        <?php if ($participant['rank'] <= 3): ?>
                          <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold <?php 
                            echo $participant['rank'] === 1 ? 'bg-yellow-200 bg-opacity-30 text-yellow-100' : 
                                 ($participant['rank'] === 2 ? 'bg-gray-200 bg-opacity-30 text-slate-100' : 'bg-orange-200 bg-opacity-30 text-orange-100'); 
                          ?>">
                    ...
                    <td class="px-6 py-4">
                      <span class="inline-flex items-center px-3 py-1 rounded-lg text-sm font-medium bg-blue-500 bg-opacity-20 text-blue-200">
                    ...
                    <td class="px-6 py-4 text-center">
                      <button onclick="viewParticipantDetails(<?php echo $participant['id']; ?>)" 
                              class="text-blue-300 hover:text-blue-400 font-medium transition-colors text-sm">
                        View Details
                      </button>
                    </td>
                  </tr>
          ...
          <?php else: ?>
            <div class="p-12 text-center">
              <svg class="mx-auto h-16 w-16 text-white mb-4 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="rounds.php" class="bg-blue-500 bg-opacity-30 hover:bg-blue-600 hover:bg-opacity-40 text-white font-medium px-6 py-3 rounded-lg transition-colors flex items-center justify-center gap-2 border border-white border-opacity-20 backdrop-blur-md">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="dashboard.php" class="bg-white bg-opacity-10 hover:bg-white hover:bg-opacity-20 text-white font-medium px-6 py-3 rounded-lg transition-colors flex items-center justify-center gap-2 border border-white border-opacity-10 backdrop-blur-md">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                  </svg>
                  Dashboard
