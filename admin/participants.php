...
if (isset($_POST['add_participant'])) {
    ...
    if (empty($full_name) || empty($number_label)) {
        ...
        // Check if number already exists within the same division
        $stmt = $conn->prepare("SELECT id FROM participants WHERE pageant_id = ? AND division_id = ? AND number_label = ?");
        $stmt->bind_param("iis", $pageant_id, $division_id, $number_label);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($result->num_rows > 0) {
            $error_message = "Participant number '$number_label' already exists in this division.";
            $show_error_alert = true;
    } else {
...
if (isset($_POST['edit_participant'])) {
    ...
    if (empty($full_name) || empty($number_label)) {
        ...
        // Check if number already exists in the target division (excluding current participant)
        $stmt = $conn->prepare("SELECT id FROM participants WHERE pageant_id = ? AND division_id = ? AND number_label = ? AND id != ?");
        $stmt->bind_param("iisi", $pageant_id, $division_id, $number_label, $participant_id);
        $stmt->execute();
        $result = $stmt->get_result();
