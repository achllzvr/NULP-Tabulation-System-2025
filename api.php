 ...
 * Public API Handler for NULP Tabulation System			$roundId = (int)($_GET['round_id'] ?? json_body()['round_id'] ?? 0);

 * Handles all public and administrative data requests			if ($roundId<=0) respond(['success'=>false,'error'=>'round_id required'],422);

 */			require_once __DIR__ . '/classes/TieService.php';

			$tieSvc = new TieService();

header('Content-Type: application/json');			$tie = $tieSvc->listBoundaryTie($roundId);

header('Access-Control-Allow-Origin: *');			respond(['success'=>true,'data'=>$tie]);

header('Access-Control-Allow-Methods: GET, POST, OPTIONS');

header('Access-Control-Allow-Headers: Content-Type');		case 'resolve_tie':

			$user = require_auth(['ADMIN']);

// Handle preflight OPTIONS request			require_csrf();

if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {			require_once __DIR__ . '/classes/TieService.php';

    http_response_code(200);			$body = json_body();

    exit();			$roundId = (int)($body['round_id'] ?? 0);
...
// Enable error reporting for debugging			$tieSvc = new TieService();

ini_set('display_errors', 1);			$res = $tieSvc->resolveBoundaryTie($roundId, $advanceIds);

ini_set('display_startup_errors', 1);			if (!empty($res['success'])) AuditLogger::log($user['user_id'], 'TIE_RESOLVE', 'round', $roundId, ['chosen'=>$advanceIds]);

error_reporting(E_ALL);			respond(['success'=> (bool)($res['success'] ?? false), 'result'=>$res]);



// Start session for admin functions		case 'public_leaderboard':

session_start();			$roundId = (int)($_GET['round_id'] ?? 0);

			if ($roundId<=0) respond(['success'=>false,'error'=>'round_id required'],422);

// Include database class			// fetch pageant from round

require_once('classes/database.php');			$pdo = Database::getConnection();

			$stP = $pdo->prepare('SELECT pageant_id FROM rounds WHERE id = ?');

// Get action from query parameter or POST data			$stP->execute([$roundId]);

$action = $_GET['action'] ?? ($_POST['action'] ?? '');			$rowP = $stP->fetch(PDO::FETCH_ASSOC);

$input = [];			if(!$rowP) respond(['success'=>false,'error'=>'round not found'],404);

			require_once __DIR__ . '/classes/VisibilityService.php';

// Parse input data			$visSvc = new VisibilityService();

if ($_SERVER['REQUEST_METHOD'] === 'POST') {			$flags = $visSvc->getFlags((int)$rowP['pageant_id']);

    $rawInput = file_get_contents('php://input');			$svc = new LeaderboardService();

    if ($rawInput) {			$rows = $svc->getRoundStandings($roundId);

        $input = json_decode($rawInput, true) ?? [];			$public = array_map(function($r) use ($flags){

    }				return [

    $input = array_merge($input, $_POST);					'participant_id'=>$flags['reveal_names'] ? $r['participant_id'] : null,

} else {					'name'=>$flags['reveal_names'] ? $r['full_name'] : 'Hidden',

    $input = $_GET;					'division'=>$flags['reveal_names'] ? $r['division'] : null,
...
function sendResponse($success, $data = null, $message = '', $error = null) {			}, $rows);

    $response = [			respond(['success'=>true,'rows'=>$public,'flags'=>$flags]);

        'success' => $success,		case 'advancement_preview':

        'message' => $message,			$user = require_auth(['ADMIN']);

        'data' => $data,			$roundId = (int)($_GET['round_id'] ?? json_body()['round_id'] ?? 0);

        'timestamp' => date('c')			if ($roundId<=0) respond(['success'=>false,'error'=>'round_id required'],422);

    ];			require_once __DIR__ . '/classes/AdvancementService.php';

    			$adv = new AdvancementService();

    if ($error) {			$data = $adv->preview($roundId);

        $response['error'] = $error;			respond(['success'=>true,'data'=>$data]);

    }

    		case 'advancement_commit':

    echo json_encode($response);			$user = require_auth(['ADMIN']);
...
// Error handler			require_once __DIR__ . '/classes/AdvancementService.php';

function handleError($message, $details = null) {			$adv = new AdvancementService();

    sendResponse(false, null, $message, $details);			$count = $adv->commit($roundId, $user['user_id']);
...
try {<?php
    ...
    $db = new database();// Unified API dispatcher

    $conn = $db->opencon();require_once __DIR__ . '/classes/AuthService.php';
